...
Another added benefit of having a dedicated `creating mechanism` is the centralization of the `domain rules` inside the `phoneNumber` function. This will prevent code duplication, avoid [[Domain Fragmentation]], and will help us enforce the [[0 Single Responsibility Principle]].
